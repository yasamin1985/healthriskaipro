...
def simulate_ou_process(chronic_score, last_year_cost, theta=0.5):
    ...
    # نمونه محاسبه در حلقه
        model = row['Model']
        age = row['Age']
        chronic = row['ChronicScore']
        cost = row['LastYearCost']
        if model == 'OU':
            predicted_cost, risk_score, risk_level = simulate_ou_process(age, chronic, cost)
        elif model == 'Linear':
            predicted_cost, risk_score, risk_level = simulate_linear_regression(age, chronic, cost)
        elif model == 'Exponential':
            predicted_cost, risk_score, risk_level = simulate_exponential_regression(age, chronic, cost)
        else:
            predicted_cost, risk_score, risk_level = 0, 0, 'Unknown'
        results.append({
            'Age': age,
