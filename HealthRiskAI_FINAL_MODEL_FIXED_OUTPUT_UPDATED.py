...
def simulate_ou_process(chronic_score, last_year_cost, theta=0.5, mu_base=1000, sigma=200):
    T = 10
    dt = 1
    x = np.zeros(T)
    x[0] = last_year_cost
...
def load_reference_table():
    url = "https://raw.githubusercontent.com/yasamin1985/healthriskaipro/refs/heads/main/disease_model_reference_FIXED2.csv"
    ref = pd.read_csv(url)
